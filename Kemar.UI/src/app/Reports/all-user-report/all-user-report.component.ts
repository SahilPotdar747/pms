...
@Component({
  selector: 'app-all-user-report',
  templateUrl: './all-user-report.component.html',
  styleUrls: ['./all-user-report.component.scss']
})
export class AllUserReportComponent {
  faSearch = faSearch;
  faTimes = faTimes;
  ...
  ngOnInit(): void {
    ...
    this.getAllUserWiseTask();
    this.getAllUserWiseTaskCountReport();
  ...
  getAllUserWiseTaskCountReport() {
    ...
    this.KemarService.get<any>(
      null,
      ServiceUrl.GetAllUserWiseTaskDataCountReport,
      query
    ).subscribe(
      (response) => {
        this.GetProjectWiseTaskcount = response;
        if (response.length > 0) {
          // temporary used completed instead of total records
  ...
  DownloadCountReport() {
    ...
    this.KemarService.get<any>(
      null,
      ServiceUrl.GetAllUserWiseTaskDataCountReportToDownload,
      query
    ).subscribe(
      (response) => {
        ...
        let insideData = {
          title: 'User Wise Task Count Summary Report',
          data: dataForExcel,
          headers: Object.keys(response[0]),
        };
        this.excel.exportExcelFoUserWiseCountReport(insideData);
      },
      (r) => {
        this.KemarService.ShowMessage('error', 'Error', r.error.errorMessage);
      }
  ...
  DownloadUserWiseTaskReport() {
    ...
    this.KemarService.get<any>(
      null,
      ServiceUrl.GetAllUserWiseReportToDownload,
      query
    ).subscribe(
      (response) => {
        let dataForExcel: unknown[][] = [];
        ...
        let insideData = {
          title: 'User Wise Task Summary Report',
          data: dataForExcel,
          headers: Object.keys(response[0]),
        };
